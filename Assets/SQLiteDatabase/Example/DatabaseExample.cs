...
public class DatabaseExample : MonoBehaviour {
	...
	void OnGUI()
	{
		...
		if (!db.Exists) 
        {
			if(GUI.Button(new Rect(Screen.width/2-125,Screen.height/2-40,250,80),"Connect/Create DB"))
			{
				CreateDB();
			}
			return;
        ...
        {
            if(btnName_CreateDeleteTable == "Delete Table" && db.DeleteTable("Questions"))
			{
				allIDs.Clear();
				allNames.Clear();
	...
	void CreateDB()
	{
		...
		//if (db.Exists) {
		//	db.CreateDatabase ("MyDatabase.db", false);
		//} else {
		//	db.CreateDatabase ("MyDatabase.db", true);
		//}
		...
		if (db.Exists) {
            ...
            db.ConnectToDefaultDatabase ("BudgetMaker.db",true);
		}
	...
	bool CreateTable()
	{
		...
		if (db.Exists) 
        {
            ...
            DBSchema schema = new DBSchema ("Questions");
			schema.AddField ("id", SQLiteDB.DB_DataType.DB_INT, 0, false, true, true);
            schema.AddField ("QuestionText", SQLiteDB.DB_DataType.DB_VARCHAR, 100, false, false, false);
	...
	void AddRow(string id, string name)
	{
		...
		// second field
        data.fieldName = "QuestionText";
		data.value = name;
		dataPairList.Add(data);
		
        // insert into Questions - first row
        int i = db.Insert("Questions",dataPairList);
	...
	void UpdateRow(string id, string name)
	{
        ...
        int i = db.Update ("Questions", dataList, condition);
	...
	void DeleteRow(string id)
	{
        ...
        int i = db.DeleteRow ("Questions", condition);
		if (i > 0)
		{
	...
	void Refresh()
	{
        ...
        // get all data from Questions table
        DBReader reader = db.GetAllData("Questions");

		while(reader != null && reader.Read())
		{
			allIDs.Add(reader.GetStringValue("id"));
            allNames.Add(reader.GetStringValue("QuestionText"));
			isTableCreated = true;
		}
